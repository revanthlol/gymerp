  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Link
          href="/staff/kiosk"
          className="glass-panel p-5 rounded-2xl border-orange-500/30 hover:border-orange-500/60 bg-gradient-to-br from-orange-500/10 to-transparent transition-all group block"
        >
        ...
        <div className="glass-panel p-5 rounded-2xl space-y-2">
          ...
          <p className="text-xs text-zinc-400">
            {data.members.length} registered members available for manual check-in.
          </p>
        </div>
        ...
        <div className="glass-panel p-5 rounded-2xl space-y-2">
          ...
          <h2 className="text-base font-semibold text-white">Manual Payments</h2>
          <p className="text-xs text-zinc-400">
            Record cash or physical POS card settlements at front counter.
          </p>
        </div>
      </div>
        ...
        <div className="divide-y divide-zinc-800/60 text-sm">
          {data.attendance.length === 0 ? (
            ...
            data.attendance.map((record) => {
              ...
              return (
                <div key={record.id} className="p-4 px-5 flex items-center justify-between hover:bg-zinc-800/20 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-emerald-950/50 border border-emerald-800/40 text-emerald-400 flex items-center justify-center">
                      <CheckCircle2 className="w-4 h-4" />
                    </div>
                    <div>
                      <p className="font-medium text-white text-sm">{member?.fullName || "Member"}</p>
                      <p className="text-xs text-zinc-500 font-mono">Token: {record.memberId.slice(0, 8)}...</p>
                    </div>
                  </div>

                  <div className="text-right font-mono text-xs text-zinc-400">
                    <p>{new Date(record.checkedInAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
